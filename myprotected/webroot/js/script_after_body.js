...
function delete_media_item_ref(id,path,table)
{
	$('#ajax-getter').html('Delete media file...');
	$.post("split/ajax/heandlers/indexHeandler.php",{action:"deleteMediaFileRef",id:id,path:path,table:table},function(data,status){
			if(status=='success')
			{
...
function usDataExc(){
	...
	$.post("split/excel/amax-report.php",{},function(data,status){
			if(status=='success')
			{
...
function selectEventType(el){
	var modal = $('[data-remodal-id=selectEventType]').remodal();
	var value = $(el).val();

	if (value != 0) {
		modal.close();
		loadPage('articles','events',74,0,'cardCreate',{eventType: value});
	}
}
	...
	$.post('split/ajax/pages/'+parent+'/'+alias+'.php',postData,function(data,status){
			...
			if(status='success')
			{
				...
				if(action_type=='cardEdit' || action_type=='cardCreate' || (action_type=='landingPage' && (parent=='all_pages' || parent=='home' || parent=='settings')) )
				{
					...
					$('.summernote').summernote({
						callbacks: {
							onImageUpload: function(files){
								var editor = this;
								sendSummernoteFiles(files, editor);
							}
						}
					});
					...
					if (alias == 'ct_genplan') {
						$('#cottages_genplan_svg .cottage_poly').click(function(){
							var ctid = $(this).data('ctid');
							var status = $(this).data('status');
							var modal = $('[data-remodal-id=cottageStatusEdit]').remodal();

							$('#ct_status_form select').val(status);
							$('#ct_status_form input[name="ct_id"]').val(ctid);

							// get textarea val
							$.post('split/ajax/heandlers/handle/handle.json.getCTtextVal.php', {id: ctid}, function(data, status){
								if (status == "success") {
									if (data.status == "success") {
										$('#ct_status_form textarea.ct_status_area').html(data.text);
										modal.open();
									}
								}
							}, 'json');
...
function cottageStatusEdit(){
	var post_data = $('#ct_status_form').serialize();
	$.post('split/ajax/heandlers/handle/handle.json.setCTVal.php', post_data, function(data, status){
		if (status == "success") {
			if (data.status == "success") {
				$('#ct_status_form')[0].reset();

				var selected_cell_id = data.sel_ct_id;


				if (data.sel_ct_status == 1) {
					$('.cottage_poly[data-ctid="'+selected_cell_id+'"]').css('fill', '#ffc000');
				}else if(data.sel_ct_status == 2){
					$('.cottage_poly[data-ctid="'+selected_cell_id+'"]').css('fill', '#6cee15');
				}else if(data.sel_ct_status == 3){
					$('.cottage_poly[data-ctid="'+selected_cell_id+'"]').css('fill', '#f33');
				}else{
					$('.cottage_poly[data-ctid="'+selected_cell_id+'"]').css('fill', '#ccc');
				}
				var modal = $('[data-remodal-id=cottageStatusEdit]').remodal();
				modal.close();
			}
...
function refreshSitemap(){
	$.post('split/ajax/heandlers/handle/handle.json.refreshSitemap.php', {}, function(data, status){
		if (status == "success") {
			if (data.status == "success") {
				$('#ajax-getter').html(data.message);
			}
...
function sendSummernoteFiles(files, editor) {
    ...
    $.ajax({
        data: data,
        type: "POST",
        url: "split/ajax/heandlers/handle/handle.json.uploadSummernoteFile.php",
        cache: false,
        contentType: false,
        processData: false,
