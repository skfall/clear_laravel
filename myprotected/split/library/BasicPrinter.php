...
class BasicPrinter
{
		...
		public function rs($query, $one=0, $update=0, $returnID=0)
		{
			global $db;
			
			return $db->q($query, $one, $update, $returnID);
		}
		...
		public function print_input($title,$name,$hold,$value,$size=25,$onchange="",$input_type="text",$inputDisabled=false, $inputReadonly=false)
		{
		...
		public function print_summernote($title,$name,$value){
			return "
			<div class='clear'></div>
           		<div class='zen-form-item' style='width: 100%;'>
					<label for='save-$name'>$title</label><br>
					<div class='zif-wrap'>
                		<textarea id='save-$name' class='my-txtarea summernote' placeholder='HTML editor' 
                        			name='$name'>$value</textarea>
                	</div>
            	</div>
            <div class='clear'></div>
		...
		public function print_multiselect($title,$currValue,$list,$fieldValue,$fieldTitle,$name,$change,$first=array(),$type=false, $cust = false)
		{
			$result = "";
			$cust_style_row1 = "";
			$cust_style_row2 = "";
			if ($cust == true) {
				$cust_style_row1 = " overflow-y: hidden; overflow-x: hidden; width: 500px; height: 200px !important; ";
				$cust_style_row2 = " width: 500px; height: 172px !important; margin-bottom: 30px; overflow-y: scroll; overflow-x: hidden; ";
			}
			$result .= "
			<div class='zen-form-item' style='$cust_style_row1'>
				<label for='save-$name'>$title </label><br>
				<div class='zif-wrap-select styled-select' style='height:100px; $cust_style_row1'>               	
					<select style='height:100px; $cust_style_row2' class='sampling_changed' id='save-$name' name='".$name."[]' multiple size='7'>
						";
						foreach($list as $item)
						{
							$in_arr = false;
		...
		public function print_image_mono($header, $field, $value, $path, $appTable ,$id)
		{
            ...
            <input type='file' name='$field' class='hidden' id='editor-file-input-$field' onChange=\"split_txt($(this).val(),'$field');\">
                
                <div class='zen-form-item'>
					<label for='save-editor-input-file-$field'>$header</label><br>
					<div class='zif-wrap'>
                		<input  id='save-editor-input-file-$field'			class='my-field' 
		...
		foreach($value as $val_item)
		{
            ...
            if(file_exists("../../../../".$path.$val_item[$mas_field]) && trim($val_item[$mas_field]) != "" || true)
			{
                ...
                /*
                	<div class='file-manager-wrap'>
						...
						<input type='text' placegolder='FILE ALT' value=\"".(isset($val_item['alt']) ? $val_item['alt'] : "Image")."\" onchange=\"save_new_file_alt(".$val_item['id'].",$(this).val(),'$field')\" />
						<label>Позиция</label>
						<input type='text' placegolder='ORDER POSITION' value=\"".(isset($val_item['order_pos']) ? $val_item['order_pos'] : "не указана")."\" onchange=\"save_new_file_order(".$val_item['id'].",$(this).val() ,'order_pos')\" />
					</div>
                */
			}
		}
		...
		public function show_card_images()
		{
			...
			?>
            	<div class="item" id="rf-item-<?php echo $rf['id'] ?>">
                	...
                	<div class="item-inside">
                    	<img alt="File not found" src=RSF."/split/<?php echo $rf['path'] ?>crop/<?php echo $rf['crop'] ?>">
                    <div class="icode hidden" contenteditable="true" 
                    	onmouseover="$(this).stop().animate({'bottom':'0px'},400,'easeInOutExpo');"
                        onmouseout="$(this).stop().animate({'bottom':'-80px'},400,'easeInOutExpo');">
			...
			foreach($list as $acc)
			{
				...
				$result .= "
							<tr id='prod_acc_ref_$ref_id'>
								<td>ID: $acc_id</td>
								<td class='img'><a class='theater' href=RSF.'/split/files/shop/products/$acc_photo' title='Аксессуар: ".str_replace("'","",$acc_name)."'><img alt='NO PHOTO' src=RSF.'/split/files/shop/products/$acc_photo' /></a></td>
								<td>$acc_name</td>
			...
			foreach($list as $acc)
			{
				...
				$result .= "
							<tr id='prod_comp_ref_$ref_id'>
								...
								<td class='img'><a class='theater' href=RSF.'/split/files/shop/products/$acc_photo' title='Товар в комплекте: ".str_replace("'","",$acc_name)."'><img alt='NO PHOTO' src=RSF.'/split/files/shop/products/$acc_photo' /></a></td>
								<td>$acc_name</td>
